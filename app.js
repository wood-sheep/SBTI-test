  ...
  "So3": {
    ...
    function getIncompleteQuestions() {
      ...
      return visibleQuestions
        ...
        .filter(item => app.answers[item.q.id] === undefined);
    }
    ...
    function updateProgress() {
      ...
      submitBtn.textContent = complete
        ? '提交并查看结果'
        : `提交并查看结果（剩 ${incomplete.length} 题）`;
      if (complete) {
        testHint.textContent = '都做完了。现在可以把你的电子魂魄交给结果页审判。';
        return;
      }

      const missingNums = incomplete.map(item => item.index);
      const preview = missingNums.slice(0, 8).join('、');
      const suffix = missingNums.length > 8 ? ' 等题' : ' 题';
      testHint.textContent = `还有 ${incomplete.length} 题未完成：第 ${preview}${suffix}`;
    }
...
const SHOP_URL = "https://apple.shbs.club";
  const AD_DURATION = 5000;

  let adAfterAction = null;
  let adFrame = null;
  let pendingIncomplete = [];

  function showAdWithProgress(afterAction, scene = "data") {
    adAfterAction = afterAction;

    const modal = document.getElementById("adModal");
    const bar = document.getElementById("adProgressBar");
    const status = document.getElementById("adProgressStatus");
    const skipBtn = document.getElementById("adSkipBtn");
    const goBtn = document.getElementById("adGoBtn");
    const sceneTextMap = {
      data: "数据加载中...",
      bank: "题库加载中...",
      result: "结果加载中..."
    };
    const loadingText = sceneTextMap[scene] || sceneTextMap.data;

    cancelAnimationFrame(adFrame);
    bar.style.width = "0%";
    status.textContent = loadingText;
    skipBtn.disabled = true;
    goBtn.disabled = false;
    goBtn.textContent = "了解一下";
    ...
    function animate(ts) {
      if (!start) start = ts;
      let p = (ts - start) / AD_DURATION;
      if (p > 1) p = 1;

      bar.style.width = (p * 100) + "%";

      if (p < 1) {
        adFrame = requestAnimationFrame(animate);
      } else {
        status.textContent = "加载完成，可点击“跳过”或“了解一下”";
  ...
  function closeAd(runAfterAction = true) {
    const modal = document.getElementById("adModal");
    modal.classList.remove("show");
    cancelAnimationFrame(adFrame);

    if (runAfterAction && typeof adAfterAction === "function") {
      const fn = adAfterAction;
      adAfterAction = null;
      fn();
    } else {
      adAfterAction = null;
  ...
  function showIncompleteModal(incomplete) {
    const modal = document.getElementById("incompleteModal");
    const summary = document.getElementById("incompleteSummary");
    const list = document.getElementById("incompleteList");
    const maxShow = 8;
    const nums = incomplete.map(item => item.index);
    const preview = nums.slice(0, maxShow).join("、");
    const suffix = nums.length > maxShow ? " 等题" : " 题";
    summary.textContent = `还有 ${incomplete.length} 题未完成：第 ${preview}${suffix}`;

    list.innerHTML = "";
    incomplete.slice(0, maxShow).forEach(item => {
      const row = document.createElement("div");
      row.className = "notice-item";

      const no = document.createElement("div");
      no.className = "notice-item-no";
      no.textContent = `第 ${item.index} 题`;

      const txt = document.createElement("div");
      txt.className = "notice-item-text";
      txt.textContent = item.q.text;
    ...
    if (incomplete.length > maxShow) {
      const tail = document.createElement("div");
      tail.className = "notice-text";
      tail.textContent = `其余 ${incomplete.length - maxShow} 题可滚动页面继续补全。`;
  ...
  function focusFirstIncomplete() {
    if (!pendingIncomplete.length) return;
    const first = questionList.children[pendingIncomplete[0].index - 1];
    if (first) {
  ...
  document.getElementById("adSkipBtn").addEventListener("click", () => {
    if (document.getElementById("adSkipBtn").disabled) return;
    closeAd(true);
  });

  document.getElementById("adGoBtn").addEventListener("click", () => {
    if (document.getElementById("adGoBtn").disabled) return;
    goShop();
  });

  document.getElementById("incompleteCloseBtn").addEventListener("click", () => {
    closeIncompleteModal();
  });

  document.getElementById("incompleteGoBtn").addEventListener("click", () => {
    closeIncompleteModal();
    focusFirstIncomplete();
  });

  document.getElementById("incompleteModal").addEventListener("click", (e) => {
    if (e.target.id === "incompleteModal") {
      closeIncompleteModal();
    }
  });

  document.getElementById("startBtn").addEventListener("click", () => {
    showAdWithProgress(() => startTest(false), "bank");
  });
  ...
  document.getElementById("submitBtn").addEventListener("click", () => {
    ...
    showAdWithProgress(() => renderResult(), "result");
  });

  window.addEventListener("load", () => {
    const box = document.querySelector(".result-actions");
    if (!box) return;

    const ad = document.createElement("div");
    ad.className = "card";
    ad.style.marginTop = "18px";
    ad.style.padding = "18px";
    ad.innerHTML = `
      <div style="font-size:18px;font-weight:800;color:#4d6a53;margin-bottom:10px;">
        🔥 ChatGPT 福利专区
      </div>
      <div style="line-height:1.9;color:#304034;">
        ChatGPT Plus 企业内部渠道 20/月<br>
        Plus共享号 23元<br>
        GPT Plus 一年200<br>
        5x Pro 月卡200 ｜ 20x Pro 月卡279<br>
        Gork / Gemini 紧缺资源
      </div>
      <div style="margin-top:10px;font-weight:700;color:#4d6a53;">
        apple.shbs.club
      </div>
    `;
    box.parentNode.appendChild(ad);
  });